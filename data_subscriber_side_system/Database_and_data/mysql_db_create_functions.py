...
def drop_exist_received_file_table_for_data_consumer(received_file_table_name):
    print('If exists, drop the received file table for data consumer')
    create_received_file_table_sql = sql.syncfile_drop_exist_recieved_file_table_for_consumer % (received_file_table_name)
    try_flag = False
    try_count = 0
    while (not try_flag) and (try_count < 5):
        try:
            ...
            print('def drop_exist_received_file_table_for_data_consumer() -> operational_error:', operational_error)
            try_count = try_count + 1
            try:
                if conn_syncfile:
                    conn_syncfile.rollback()
            except Exception as exception_error:
                print('def drop_exist_received_file_table_for_data_consumer() -> operational_error -> exception_error', exception_error)
            continue
        except Exception as exception_error:
            try_count = try_count + 1
            print('def drop_exist_received_file_table_for_data_consumer() -> exception_error', exception_error)
            continue
    if try_count < 5:
        return True
    else:
...
def create_received_file_table_for_data_consumer(received_file_table_name):
    ...
    while (not try_flag) and (try_count < 5):
        try:
            ...
            print('def create_received_file_table_for_data_consumer() -> operational_error:', operational_error)
            try_count = try_count + 1
            try:
                if conn_syncfile:
                    conn_syncfile.rollback()
            except Exception as exception_error:
                print('def create_received_file_table_for_data_consumer() -> operational_error -> exception_error',
                      exception_error)
            continue
        except Exception as exception_error:
            try_count = try_count + 1
            print('def create_received_file_table_for_data_consumer() -> exception_error', exception_error)
            continue
    if try_count < 5:
        print('the received file table:%s has been created successfully'%received_file_table_name)
        return True
    else:
        print('the received file table:%s has been created unsuccessfully'%received_file_table_name)
        return False
